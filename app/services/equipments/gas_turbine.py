...
class GasTurbine:
  def __init__(self, input, gas_fuel, substance_repo, icph_repo):
    ...
    self.input_air = InputAir(self.input, substance_repo)
  ...
  def input_air_mass_flow(self):
    ...
    saturation_pressure = self.saturation_parameters.saturation_pressure()
    absolute_humidity = self.humidity.absolute_humidity(saturation_pressure, self.input.local_atmospheric_pressure, self.input.relative_humidity)
    air_molar_flow = self.input_air.molar_flow_calc(oxygen_stoichiometric, absolute_humidity)
    print(air_molar_flow)

    return
  
  def combustion_gas_mass_flow():
    return
